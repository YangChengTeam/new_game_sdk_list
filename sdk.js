...
function d(apkpath, output) {
    ...
    return exec(`${setting.java} -jar build_tools${path.sep}apktool.jar d ${apkpath} -o ${output}`);
}
...
async function main(callback) {
    ...
    await sign()
    callback(`I: 打包成功->${setting.publishDir}${gameinfo.name}_signed.apk`)
    if (fs.existsSync(`${setting.publishDir}${gameinfo.name}_signed.apk`)) {
        console.log(`打包成功: ${setting.publishDir}${gameinfo.name}_signed.apk`)
    }
}
...
async function mergeAndroidManifestXml(sdkpath, gamepath) {
    ...
    if (sdknode && gamenode) {
        ...
        if (sdkmanifest.application[0].$[androidInfo.androidNameKey] && !gamemanifest.application[0].$[androidInfo.androidNameKey]) {
            gamemanifest.application[0].$[androidInfo.androidNameKey] = sdkmanifest.application[0].$[androidInfo.androidNameKey];
        }
...
function rFileHeader(type, packageName) {
    packageName = packageName.replace(new RegExp("\\.", "g"), "/")
...
function writeRFile(gamenodes) {
    ...
    for (var i = 0; i < gamenodes.length; i++) {
        ...
        if (!fs.pathExistsSync(rfilePath)) {
            ...
            fs.writeFileSync(rfilePath, rFileHeader(type, sdkinfo.packageName))
        }
