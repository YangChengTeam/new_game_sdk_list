...
function pretreatment(sdkdir, gamedir) {
    ...
    ['com/feiyou', 'com/unionpay'].forEach(name=>{
        var sourcePath = path.join(gamedir, 'unknown', name.replace("/", path.sep))
        if (fs.pathExistsSync(sourcePath)) {
            fs.removeSync(sourcePath)
            console.log(sourcePath)
        }
    ...
    templateRes = path.join(setting.tempDir, 'template', 'res')
    fs.readdirSync(templateRes).map(dir => {
        var sourcePath = path.join(templateRes, dir)
        fs.readdirSync(sourcePath).map(name => {
            sourcePath = path.join(gamedir, 'res', dir, name)
            if (fs.pathExistsSync(sourcePath)) {
                fs.removeSync(sourcePath)
                console.log(sourcePath)
            }
...
async function mergePublicXml(sdkpath, gamepath) {
    ...
    for (var j = 0; j < glen; j++) {
        ...
        var sourcePath = path.join(sdkinfo.gamedir, 'res', gnode.$['type'] + "s", gnode.$['name'])
        if (fs.pathExistsSync(path.join(sourcePath, '.xml')) 
        || fs.pathExistsSync(path.join(sourcePath, '.png')) 
        || fs.pathExistsSync(path.join(sourcePath, '.jpg'))
        || fs.pathExistsSync(path.join(sourcePath, '.jpeg')) 
        || fs.pathExistsSync(path.join(sourcePath, '.webp'))) {
            gamenodes.splice(j, 1)
            glen -= 1
            console.log(gnode.$['type'] +'-'+  gnode.$['name'])
        }          
    }
