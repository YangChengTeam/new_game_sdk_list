...
async function mergePublicXml(sdkpath, gamepath) {
    ...
    for (var j = 0; j < glen; j++) {
        ...
        if (fs.pathExistsSync(sourcePath + '.xml') 
        || fs.pathExistsSync(sourcePath+ '.png') 
        || fs.pathExistsSync(sourcePath+ '.jpg')
        || fs.pathExistsSync(sourcePath+ '.jpeg')) 
        || fs.pathExistsSync(sourcePath+ '.webp')) {
