...
function pretreatment(sdkdir, gamedir) {
    ...
    ['okhttp3', 'okio', 'com/ipaynow', 'com/UCMobile', 'com/unionpay', 'com/ta', 'com/ut'
    , 'com/feiyou', 'com/game', 'com/vqs', 'com/squareup', 'com/umeng'].forEach(forEach(name=>{
        sourcePath = path.join(gamedir, 'smali', name)
        if (fs.pathExistsSync(sourcePath)) {
            fs.removeSync(sourcePath)
        }
    }))

    fs.readdirSync(gamedir).map(dir => {
        ['libuptsmaddon.so', 'libentryex.so', 'libplugin_phone.so', 'libfyae.so', 'libentryexpro.so'].forEach(name=>{
            sourcePath = path.join(gamedir, 'smali', name)
            if (fs.pathExistsSync(sourcePath)) {
                fs.removeSync(sourcePath)
            }
